...
| App      | Task Name                | Description                               | Prompt                                                                                                                                                                      | Context Data |
|----------|--------------------------|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| Obsidian | task01_createNote        | Create a new note in the vault            | Create a new note named '${note_name}' in the Obsidian vault at /workspace/.mcpworld/obsidian/vault. Ensure the note exists with any content you deem appropriate, and confirm it was created before finishing. | `tests/context_data/obsidian/sample-vault` |
| Obsidian | task02_deleteNote        | Delete the pre-existing Archive note      | Delete the note located at '${note_relative_path}' inside the Obsidian vault at /workspace/.mcpworld/obsidian/vault. Confirm it is removed without deleting any other files.                                     | ArchiveMe.md |
| Obsidian | task03_appendToNote      | Append a summary block to ProjectPlan     | Open '${note_relative_path}' inside the Obsidian vault at /workspace/.mcpworld/obsidian/vault and append the text: ${append_text}. Do not remove or rewrite the existing content; only add to the end.           | ProjectPlan.md |
| Obsidian | task04_renameNote        | Rename MeetingNotes while preserving text | Rename the note '${source_note}' inside /workspace/.mcpworld/obsidian/vault to '${target_note}'. Ensure the new note keeps the original contents.                                                                | MeetingNotes.md |
| Obsidian | task05_createDailyNote   | Create/replace the daily template note    | Create '${note_relative_path}' inside /workspace/.mcpworld/obsidian/vault using the exact template text below (no extra lines). If a file already exists at that path, replace it so the final note matches line-for-line:\n${template_text} | Daily/Daily-2025-01-01.md |
| Obsidian | task06_moveNote          | Move Ideas.md into Archive                | Move the note '${source_note}' at /workspace/.mcpworld/obsidian/vault into the Archive folder so that it exists at '${target_relative_path}'. Do not duplicate the file—perform an actual move.                 | Ideas.md |
| Obsidian | task07_extendAgenda      | Add action items to Weekly Agenda         | Open '${note_relative_path}' inside the Obsidian vault at /workspace/.mcpworld/obsidian/vault. First review the existing agenda section, then append the following action-item block exactly as written:\n${append_text}\nDo not remove any existing content; simply add the block to the end of the file. | Meetings/Agenda.md |
| Obsidian | task08_updateHabitTracker | Update the meditation count               | Locate '${note_relative_path}' (use a search tool if needed), confirm the current habit counts, and update the meditation entry so it reads '${updated_line}' while keeping the other habits unchanged. The final note should contain the same heading and list, except for that single value change. | HabitTracker.md |
| Obsidian | task09_linkedNote        | Create a note with backlink to Ideas      | Confirm that 'Ideas.md' exists, then create a new note at '${note_relative_path}' containing the title '${note_title}', the summary sentence below, and a wiki-link to ${link_target}:\n${summary_text}\nMake sure the link syntax is correct so Obsidian registers the backlink. | Ideas.md |
| Obsidian | task10_extractTimeline   | Extract Upcoming Milestones into new note | Inspect '${source_note}' to locate the '${section_heading}' section, then create a new note at '${target_note}' that contains only that section and its bullet list. Keep the section in the original file as well. | Projects/ProjectPlan.md |
