...
PC_CANARY_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "PC-Canary")
)
...
try:
    ...
    print(
        f"错误: 无法导入 computer_use_demo 组件。请确保脚本在正确的环境中运行，或者已将项目添加到 PYTHONPATH。"
    )
    print(f"原始错误: {e}")
    sys.exit(1)
...
try:
    ...
    print(
        f"错误: 无法导入 PC-Canary Evaluator 组件。请确保 PC-Canary 路径正确并已添加到 PYTHONPATH。"
    )
    print(f"原始错误: {e}")
    sys.exit(1)
...
async def run_agent_loop(args, evaluator: BaseEvaluator):  # <-- 接收 evaluator 实例
    ...
    def is_timeout():
        return args.timeout > 0 and time.time() - start_time > args.timeout

    while (
        args.max_turns is None or turn_count < args.max_turns
        ...
        evaluator.record_event(
            ...
            {"timestamp": llm_start_time, "model_name": model_name_to_record},
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run Computer Use Demo Agent Headlessly with Evaluator"
    )
    # Agent 参数
    parser.add_argument(
        "--api_key",
    ...
    parser.add_argument(
        "--tool_version",
        type=str,
        default=available_tool_versions[0],
        choices=available_tool_versions,
        help="Version of tools to use",
    )
    parser.add_argument(
        "--max_tokens", type=int, default=4096, help="Max tokens for model response"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Maximum number of conversation turns (user + assistant, default: 10)",
    )
    # Evaluator 参数
    parser.add_argument(
        "--task_id",
        type=str,
        required=True,
        help="PC-Canary Task ID (format: category/id, e.g., computeruse/task01_example)",
    )
    parser.add_argument(
        "--log_dir",
        type=str,
        default="logs_computer_use_eval",
        help="Directory for evaluator logs and results",
    )
    parser.add_argument(
        "--app_path",
        type=str,
        default=None,
        help="Path to specific application if required by the task",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=300,
        help="Overall execution timeout in seconds (default: 300)",
    )
    parser.add_argument(
        "--exec_mode",
        type=str,
        choices=["mixed", "gui", "api"],
        default="mixed",
    ...
    if not (args.api_key or os.getenv("ANTHROPIC_API_KEY")):
        print(
            "错误: 必须提供 Anthropic API 密钥 (--api_key 或 ANTHROPIC_API_KEY 环境变量)"
        )
        sys.exit(1)
