...
## 12/?? VS Code Task Localization & Sandbox Prep
- Translated task03–task25 VS Code configs, hookers, and handlers to English so evaluator messaging is consistent with the new task01/task02 format.
- Updated every task config to launch `/workspace/PC-Canary/apps/vscode/scripts/code.sh` with `--user-data-dir=/workspace/.mcpworld/...` and copied context data into `/workspace/.mcpworld/vscode/...` to keep all modifications inside the sandbox.
- Normalized hookers/handlers to reference `/workspace/.mcpworld` paths (instead of `/root`), emit English log strings, and when needed capture before/after snapshots for evaluation.
- Added parameter fixes where handlers previously ignored config inputs (e.g., task11 autosave, task18 plugin value) so future tweaks only require editing the JSON.
...
- 2025-11-10: Hardened tool-result serialization in `computer-use-demo/computer_use_demo/loop.py` so Anthropic errors no longer 400 when a tool fails: error segments now always include plain-text details (with a fallback message) and omit screenshots when `is_error` is true (follow-up tweak keeps screenshot outputs in the non-error path by only treating non-empty `error` strings as failures).
