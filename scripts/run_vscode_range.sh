...
usage() {
    echo "Usage: $0 <start_task_number> <end_task_number>" >&2
}
...
SUMMARY_FILE="$LOG_ROOT/vscode_batch_summary.csv"
METRICS_FILE="$LOG_ROOT/vscode_metrics.csv"
...
mapfile -t TASK_DIRS < <(find PC-Canary/tests/tasks/vscode -maxdepth 1 -mindepth 1 -type d -name "task??_*" | sort)
...
echo ">>> Running vscode tasks $START through $END (of $TOTAL_TASKS total)"
