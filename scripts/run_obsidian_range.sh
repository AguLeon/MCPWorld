...
usage() {
    echo "Usage: $0 <start_task_number> <end_task_number> [log_root]" >&2
}
...
if [[ -n "$CUSTOM_LOG_ROOT" ]]; then
    ...
    LOG_ROOT=${LOG_ROOT:-logs_computer_use_eval/obsidian_runs}
fi

SUMMARY_FILE="$LOG_ROOT/obsidian_batch_summary.csv"
METRICS_FILE="$LOG_ROOT/obsidian_metrics.csv"
...
mapfile -t TASK_DIRS < <(find PC-Canary/tests/tasks/obsidian -maxdepth 1 -mindepth 1 -type d -name "task??_*" | sort)
...
echo ">>> Running obsidian tasks $START through $END (of $TOTAL_TASKS total)"

for idx in $(seq "$START" "$END"); do
    task_dir=${TASK_DIRS[$((idx - 1))]}
    task_name=$(basename "$task_dir")
    TASK_ID="obsidian/$task_name"
